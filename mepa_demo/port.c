...
static void port_setup(mesa_port_no_t port_no, mesa_bool_t aneg, mesa_bool_t init)
{
    ...
    conf.sd_enable = 0; //(cap & MEBA_PORT_CAP_SD_ENABLE ? 1 : 0);
...
static void cli_cmd_phy_spi_read(cli_req_t *req)
{
    ...
    uint32_t value;
    port_cli_req_t         *mreq = req->module_req;
    //spi_reg_read(NULL, mreq->offset, &value);
    //spi_reg_read(NULL, mreq->offset, &value);
    spi_reg_read(0, mreq->offset, &value);
    spi_reg_read(0, mreq->offset, &value);
...
#if 0
static void cli_cmd_phy_spi_write(cli_req_t *req)
{
    uint8_t SFP_V_START = 20;
    mesa_port_no_t         uport, iport;
    uint32_t value;
    port_cli_req_t         *mreq = req->module_req;
    //spi_reg_write(NULL, mreq->offset, mreq->val);
    //spi_reg_read(NULL, mreq->offset, &value);
...
static void cli_cmd_phy_scan(cli_req_t *req)
{
    ...
    for (miim_ctrl = miim_start; miim_ctrl <= miim_end; miim_ctrl++) {
        ...
        for (adr = 0; adr < 32; adr++) {
            if (mesa_mmd_read(NULL, 0, miim_ctrl, adr, 0x1e, 0, &value) == MESA_RC_OK) {
                cli_printf("Clause 45: Ctrl:%d MMD addr:1,%-2d - Found Phy 0x%x (reg 3)\n",miim_ctrl, adr, value);
                found_mmd = TRUE;
            }
        }
...
static cli_cmd_t cli_cmd_table[] = {
    ...
    },
    {
        "spi read [offset] ",
        "spi read ",
        cli_cmd_phy_spi_read
    },   
#if 0 
    {
        "spi write [offset] [val] ",
        "spi write ",
        cli_cmd_phy_spi_write
     },
  #endif
    {
...
static int cli_parm_offset(cli_req_t *req)
{
    port_cli_req_t *mreq = req->module_req;
    return cli_parm_u32(req, &mreq->offset, 0, 0xffffffff);
  //  cli_printf(" offset: %x\n", mreq->offset);
  //  return 0;
}

static int cli_parm_value(cli_req_t *req)
{
    port_cli_req_t *mreq = req->module_req;
    return cli_parm_u32(req, &mreq->val, 0, 0xffffffff);
  //  cli_printf(" offset: %x\n", mreq->offset);
  //  return 0;
}
