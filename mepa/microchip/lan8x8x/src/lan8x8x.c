...
static mepa_rc lan8x8x_phy_cfg_clr(mepa_device_t *const dev)
{
    ...
    MEPA_RC_GOTO(rc, lan8x8x_setup_lpbk(dev, NULL));

    //Setup events
    MEPA_RC_GOTO(rc, lan8x8x_int_events_set(dev, data->events, PHY_TRUE));
...
static mepa_rc lan8x8x_phy_reset(mepa_device_t *dev)
{
    ...
    //clear config
    MEPA_RC_GOTO(rc, lan8x8x_phy_cfg_clr(dev));
                                             ...
                                             const mepa_gpio_conf_t *const gpio_conf)
{
    ...
    if ((mode == MEPA_GPIO_MODE_IN) || (mode == MEPA_GPIO_MODE_OUT)) {
        ...
        phy_data_t *const data = (phy_data_t *const)dev->data;
        MEPA_RC_GOTO(rc, lan8x8x_config_led(dev, gpio_conf));
        //updata metadata
    ...
    if (gpio_no < 3U) {
        if (gpio_en) {
            ...
            dir = dir << gpio_no;
            MEPA_RC_GOTO(rc, phy_mmd_reg_modify(dev, MDIO_MMD_VEND1, LAN8X8X_GPIO_DIR, dir, dir));
        } else if (clk_en) {
            val = ((uint16_t)1U << (8U + gpio_no));
        } else {
            /*  Event misra_c_2012_rule_15_7_violation: No non-empty terminating "else" statement. */
        }
        MEPA_RC_GOTO(rc, phy_mmd_reg_modify(dev, MDIO_MMD_VEND1, LAN8X8X_REG_CONTROL1,
...
static mepa_rc lan8x8x_pma_baset1_setup_aneg(mepa_device_t *const dev)
{
    ...
    MEPA_RC_GOTO(rc, phy_mmd_reg_modify(dev, MDIO_MMD_AN, MDIO_AN_T1_ADV_M,
                                        adv_r2_mask, adv_r2));
     ...
     */
    MEPA_RC_GOTO(rc, phy_mmd_reg_wr(dev, MDIO_MMD_AN, MDIO_AN_T1_ADV_L,
                                    adv_r1));
...
static mepa_rc lan8x8x_phy_setup(mepa_device_t *const dev)
{
    ...
    if (data->conf.speed == MESA_SPEED_AUTO) {
        ...
        MEPA_RC_GOTO(rc, lan8x8x_config_done(dev, PHY_FALSE));
    } else {
...
static mepa_rc lan8x8x_cable_test_report(mepa_cable_diag_result_t *res,
                                         uint16_t cd_sts)
{
    uint16_t location = TC12_CD_LOC(cd_sts);
    uint16_t status = TC12_CD_STS(cd_sts);

    /* Set status */
    switch (status) {
    case 0:
    case TC12_HDD_TDR_LINK_UP:
    case TC12_HDD_TDR_CBL_OK:
        res->status[TC12_HDD_TDR_PAIR_0] = MESA_VERIPHY_STATUS_OK;
        break;
    case TC12_HDD_TDR_CBL_SHORT:
        res->status[TC12_HDD_TDR_PAIR_0] = MESA_VERIPHY_STATUS_SHORT;
        break;
    case TC12_HDD_TDR_CBL_OPEN:
        res->status[TC12_HDD_TDR_PAIR_0] = MESA_VERIPHY_STATUS_OPEN;
        break;
    case TC12_HDD_TDR_TST_ACTIVE:
        res->status[TC12_HDD_TDR_PAIR_0] = MESA_VERIPHY_STATUS_RUNNING;
        break;
    default:
        res->status[TC12_HDD_TDR_PAIR_0] = MESA_VERIPHY_STATUS_UNKNOWN;
        break;
    }

    if ((status == TC12_HDD_TDR_CBL_OPEN) ||
        (status == TC12_HDD_TDR_CBL_SHORT)) {
        /* Set length which is in meters */
        switch (location) {
        case TC12_HDD_TDR_CD_NO_ERR:
            res->length[TC12_HDD_TDR_PAIR_0] = TC12_HDD_TDR_LEN_0_M;
            break;
        case TC12_HDD_TDR_CD_LOC1:
            res->length[TC12_HDD_TDR_PAIR_0] = TC12_HDD_TDR_LEN_2_M;
            break;
        case TC12_HDD_TDR_CD_LOC2:
            res->length[TC12_HDD_TDR_PAIR_0] = TC12_HDD_TDR_LEN_3_M;
            break;
        case TC12_HDD_TDR_CD_LOC3:
            res->length[TC12_HDD_TDR_PAIR_0] = TC12_HDD_TDR_LEN_5_M;
            break;
        case TC12_HDD_TDR_CD_LOC4:
            res->length[TC12_HDD_TDR_PAIR_0] = TC12_HDD_TDR_LEN_6_M;
            break;
        case TC12_HDD_TDR_CD_LOC5:
            res->length[TC12_HDD_TDR_PAIR_0] = TC12_HDD_TDR_LEN_8_M;
            break;
        case TC12_HDD_TDR_CD_LOC6:
            res->length[TC12_HDD_TDR_PAIR_0] = TC12_HDD_TDR_LEN_9_M;
            break;
        case TC12_HDD_TDR_CD_LOC7:
            res->length[TC12_HDD_TDR_PAIR_0] = TC12_HDD_TDR_LEN_11_M;
            break;
        case TC12_HDD_TDR_CD_LOC8:
            res->length[TC12_HDD_TDR_PAIR_0] = TC12_HDD_TDR_LEN_12_M;
            break;
        case TC12_HDD_TDR_CD_LOC9:
            res->length[TC12_HDD_TDR_PAIR_0] = TC12_HDD_TDR_LEN_14_M;
            break;
        case TC12_HDD_TDR_CD_LOC10:
            res->length[TC12_HDD_TDR_PAIR_0] = TC12_HDD_TDR_LEN_15_M;
            break;
        case TC12_HDD_TDR_CD_LOC11:
        default:
            res->length[TC12_HDD_TDR_PAIR_0] = TC12_HDD_TDR_LEN_MAX_M;
            break;
        }
    }
...
static mepa_rc lan8x8x_phy_cable_diag_start(mepa_device_t *dev)
{
    ...
    if ((data->init_done) && (data->conf.admin.enable)) {
        ...
        MEPA_RC_GOTO(rc, lan8x8x_phy_config(dev, MESA_SPEED_100M, PHY_TRUE));
        ...
        //cable diag report
        MEPA_RC_GOTO(rc, lan8x8x_cable_test_report(res, cd_sts));
    }
...
static mepa_rc lan8x8x_cable_diag_start_int(mepa_device_t *dev)
{
    ...
    MEPA_RC_GOTO(rc, lan8x8x_poll_int(dev, &link_status));
